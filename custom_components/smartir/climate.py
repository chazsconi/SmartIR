...
class SmartIRClimate(ClimateEntity, RestoreEntity):
    ...
    async def async_set_hvac_mode(self, hvac_mode):
        ...
        _LOGGER.warn(f"set_hvac_mode mode: {hvac_mode}")
    ...
    async def send_command(self):
        async with self._temp_lock:
            try:
                ...
                _LOGGER.warn(f"send_command power_sensor x: {self._current_power_sensor_state} operation_mode: {operation_mode} fan_mode: {fan_mode} swing_mode: {swing_mode} temp: {target_temperature}")

                if operation_mode.lower() == HVAC_MODE_OFF and 'off' in self._commands:
                    _LOGGER.warn("send_command sending 'off'")
                    await self._controller.send(self._commands['off'])
                    return

                if 'on' in self._commands:
                    _LOGGER.warn("send_command sending 'on'")
                    await self._controller.send(self._commands['on'])
                    await asyncio.sleep(self._delay)

                if 'toggle' in self._commands:
                    _LOGGER.warn("'toggle' command present")
                ...
                _LOGGER.warn("send_command for mode/fan/swing/temp")
    ...
    async def _async_power_sensor_changed(self, entity_id, old_state, new_state):
        ...
        _LOGGER.warn(f"power_sensor_changed old: {old_state.state} new: {new_state.state}")
        if new_state is None:
            return
